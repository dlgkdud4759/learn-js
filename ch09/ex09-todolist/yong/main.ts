...
async function removeItem(no: number) {
  try {
    ...
    if (err instanceof AxiosError) {
      console.error("삭제 실패", err.response?.data.message);
    }
  }
}
...
async function toggleDone(no: number) {
  ...
  try {
    await axios.patch<TodoInfoRes>(`${API_SERVER}/todolist/${no}`, {
      done: isDone,
    });
    showList();
  } catch (err) {
    if (err instanceof AxiosError) {
      console.error("수정 실패", err.response?.data.message);
    }
  }
}
