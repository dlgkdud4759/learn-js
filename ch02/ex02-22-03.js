...
// id=2인 할일
// Array.prototype.find: 콜백 함수가 true를 리턴하는 첫 번째 요소를 반환
let todo = todoList.find((todoItem) => todoItem.id === 2);

console.log("id=2인 할일", todo);

// id=3인 할일의 index
// Array.prototype.findIndex: 콜백 함수가 true를 리턴하는 첫 번째 요소를 반환
...
// 남은 할일이 하나라도 있는가?
// Array.prototype.some: 콜백 함수가 true를 리턴하는 경우가 있으면 true를 반환
let hasTodo = todoList.some((todoItem) => !todoItem.done);

console.log("남은 할일이 하나라도 있는가?", hasTodo);

// 할일이 모두 완료 되었는가?
// Array.prototype.every: 콜백 함수가 전부 true를 리턴하는 경우 true를 반환
...
console.log("완료된 할일 title", doneTitleList);
