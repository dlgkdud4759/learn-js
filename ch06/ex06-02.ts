...
(() => {
  let str: string = "HaYeong";
  let age: number = 25;
  let done: boolean = true;
  // done = 'false'; // 컴파일 에러(boolean 타입에 문자열을 작성함)

  let nullVal: null = null; // null 값 밖에 가질 수 없다.
  let emptyVal: undefined;

  let todo: object = { title: "TypeScript 공부", done: false }; // 객체 타입
  let todoList1: Array<string> = ["JavaScript 복습", "TypeScript 공부"]; // 배열(문자열)
  let todoList2: string[] = ["JavaScript 복습", "TypeScript 공부"]; // 배열(문자열)
  ...
  let userName1: any = "오이영"; // 모든 타입을 의미하지만, TypeScript를 사용하지 않는다는 의미
  userName1 = 520;
